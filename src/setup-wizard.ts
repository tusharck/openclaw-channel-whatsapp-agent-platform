...
/**
 * Shape `defineTokenCredential` requires: `config` is the account's raw config
 * block, which it inspects for a configured value.
 */
...
function resolveWizardAccount(params: { cfg: OpenClawConfig; accountId: string }): WizardAccount {
  return {
...
export const whatsappAgentPlatformSetupWizard: ChannelSetupWizard = {
  channel: CHANNEL_ID,
  ...
  credentials: [
    defineTokenCredential<WizardAccount>({
      inputKey: "apiKey",
      configKey: "apiKey",
      ...
      patchAccount: ({ cfg, accountId, patch, clearFields }) =>
        patchAccountConfigBlock(cfg, accountId, patch, clearFields),
      set: { value: "input" },
      useEnv: {},
    }),
  ],
