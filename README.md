...
- **Outbound is capped at 12 messages/min** (rolling 60s window, per agent).
  Read receipts + typing, update polls, and media each have their own limits
...
| `sendReadReceipts` | `true` | Send a read receipt + typing indicator while working. |
...
setup-entry.ts        defineSetupPluginEntry + createOptionalChannelSetupSurface
openclaw.plugin.json  manifest (regenerated by `openclaw plugins build`)
src/
...
6. **`package.json > openclaw.channel` is `{ id, label, blurb }`.**
7. **zod v4.** The SDK builds config schemas with zod v4 (4.4.x); this plugin
   pins `zod` to match so the schema types unify.
8. **Offset persistence is file-backed** (`src/plugin-state.ts`) because the
   SDK's plugin-state store is not typed for external use; the `OffsetStore`
   interface makes it swappable.
9. **No declaration emit.** A channel plugin is loaded from compiled JS; shipping
   `.d.ts` would require naming the SDK's internal bundle types, so `declaration`
   is off.
10. **Manifest is generated, not built by the CLI.** `openclaw plugins build`
