...
const config = {
    listAccountIds: (cfg) => {
        const block = readChannelBlock(cfg);
...
async function stopActiveRun(key, log) {
...
const gateway = {
    startAccount: async (ctx) => {
        ...
        void gcMediaDir(stateRoot)
            .then((r) => {
            if (r.removed > 0)
                logger.info?.(`Media GC removed ${r.removed} file(s), freed ${Math.round(r.freedBytes / 1024)}KB.`);
        })
            .catch(() => undefined);
        const sink = {
            ...
            onFatal: (err) => {
                ...
                if (health)
                    health.fatalReason = err.message;
                try {
                    ctx.setStatus({ ...ctx.getStatus(), state: "error", detail: err.message });
                }
                catch {
        ...
        const done = poller
            ...
            if (health) {
                ...
                if (poller.stoppedBy)
                    health.fatalReason = poller.stoppedBy.message;
            }
            if (activeRuns.get(key)?.controller === controller)
                activeRuns.delete(key);
        });
...
const heartbeat = {
    ...
    checkReady: async ({ cfg, accountId }) => {
        ...
        if (!account.enabled)
            return { ok: false, reason: "Account is disabled in config." };
        const health = pollerHealth.get(accountKey(account.accountId));
        if (!health)
            return { ok: true, reason: "Configured; poll loop not started yet." };
        if (health.fatalReason)
            return { ok: false, reason: health.fatalReason };
